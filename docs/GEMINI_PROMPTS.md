# DUGate: Enterprise-Grade Prompts for 15 Gemini Connectors

Dưới đây là bộ **15 Prompts nâng cao** dành riêng cho kiến trúc của DUGate kết nối với LLM Gemini. Bộ prompt này áp dụng kỹ thuật **System Instruction phân tách bằng thẻ XML** để buộc Gemini phải tuân thủ nghiêm ngặt các tham số, đóng khung ngữ cảnh (Closed-book constraint) nhằm **TRIỆT TIÊU TỐI ĐA TRẠNG THÁI HALLUCINATION (Bịa đặt thông tin)**, quy định rõ cách xử lý với dữ liệu bị thiếu (`null`, `[illegible]`, `NOT_FOUND`).

---

## 1. Nhóm Ingest (Thu thập & Số hóa định dạng)

### 1.1 Document Layout Parser (`ext-doc-layout`)
- **System Prompt:**
```xml
<role>
...
### 1.2 Handwriting Vision Reader (`ext-vision-reader`)
- **System Prompt:**
```xml
<role>
...
## 2. Nhóm Extract (Trích xuất dữ liệu)

### 2.1 Structured Data Extractor (`ext-data-extractor`)
- **System Prompt:**
```xml
<role>
...
## 3. Nhóm Analyze (Phân tích dữ liệu)

### 3.1 Document Classifier (`ext-classifier`)
- **System Prompt:**
```xml
<role>
...
### 3.2 Sentiment Analyzer (`ext-sentiment`)
- **System Prompt:**
```xml
<role>
...
### 3.3 Compliance Checker (`ext-compliance`)
- **System Prompt:**
```xml
<role>
...
### 3.4 Fact Verifier (`ext-fact-verifier`)
- **System Prompt:**
```xml
<role>
...
### 3.5 Quality & Risk Evaluator (`ext-quality-eval`)
- **System Prompt:**
```xml
<role>
...
## 4. Nhóm Transform (Biến đổi nội dung)

### 4.1 Document Translator (`ext-translator`)
- **System Prompt:**
```xml
<role>
...
### 4.2 Content Rewriter (`ext-rewriter`)
- **System Prompt:**
```xml
<role>
...
### 4.3 PII Redactor & Template Filler (`ext-redactor`)
- **System Prompt:**
```xml
<role>
...
## 5. Nhóm Generate (Tạo sinh & Sáng tạo)

### 5.1 Content Generator (`ext-content-gen`)
- **System Prompt:**
```xml
<role>
...
### 5.2 Document QA Engine (`ext-qa-engine`)
- **System Prompt:**
```xml
<role>
...
## 6. Nhóm Compare (Đối chiếu tài liệu)

### 6.1 Document Comparator (`ext-comparator`)
- **System Prompt:**
```xml
<role>
...
## 7. Nhóm System (Trợ lý điều phối hệ thống)

### 7.1 DUGate System Assistant (`sys-assistant`)
- **System Prompt:**
```xml
<role>
